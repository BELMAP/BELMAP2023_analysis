#-------------7. AMR in food producing animals ------------------------------------
...
Indicator_vet_data <- read_csv2("\\\\sciensano.be/FS/1220_BactDis_Scientist/Scientists/Moira/BELMAP 2023/Data/2.AMR/Food_producing/Indicator-VET_data_combined.csv",
                                col_types = cols(
                                  "Year" = col_double(),
...
write_csv(start_data_frame,file = "7_food_producing_data_and_GLM_predictions.csv")
...
for(i in combined_categories){
  ...
  output_file_name <- paste("7_food_producing_GLM_output_summaries/GLM_output_",i,".txt",sep="") 
  ...
  dispersion_graph_file<-paste("7_food_producing_Dispersion_graphs/Dispersion_graph_poisson_model",i,".pdf",sep="") 
  ...
  predicted_dataset_filename <- paste("7_food_producing_GLM_predictions/GLM_predictions_",i,".csv",sep="") 

  write_csv(predict_data,file = predicted_dataset_filename)  
  write_csv(graph_data,file = "7_food_producing_data_and_GLM_predictions.csv", append = TRUE)
...
graph_data_complete <- read.csv("7_food_producing_data_and_GLM_predictions.csv") %>%
...
(E_coli_graph <- graph_data_complete_labels %>%
  filter(Pathogen == "E. coli") %>%
    ...
    facet_wrap(Level_indicator ~ Host, nrow = 2)+
